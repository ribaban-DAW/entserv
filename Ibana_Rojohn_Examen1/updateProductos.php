...
if ($_SERVER["REQUEST_METHOD"] == "POST") { // Viene del mismo archivo
    ...
    try {
        ...
        $query = "UPDATE employees SET productName = :productName, lastName = :lastName, extension = :extension, email = :email, officeCode = :officeCode, reportsTo = :reportsTo, jobTitle = :jobTitle, pass = :pass WHERE employeeNumber = :employeeNumber";
    ...
    $conn = getDBConnection();
    $query = "SELECT productName, lastName, extension, email, officeCode, reportsTo, jobTitle, pass FROM employees WHERE employeeNumber = :employeeNumber";
    $stmt = $conn->prepare($query);
...
<body>
    ...
    <form method="POST">
        ...
        <div>
            ...
            <input id="productName" name="productName" type="text" value="<?php echo $currentEmployee["productName"]; ?>">
        </div>
        <div>
            ...
            <select name="reportsTo" id="reportsTo">
                <?php
                    ...
                    foreach ($employees as $employee) {
                        $output .= "<option value=\"${employee["employeeNumber"]}\">${employee["productName"]} ${employee["lastName"]}</option>";
                    }
                    echo $output;
                    ?>
            </select>
